...
export class NewsService {
    ...
    joinGroup(equity: string): void {
        if (this._hubConnection) {
            this._hubConnection.invoke('JoinGroup', equity);
        }
    }

    leaveGroup(equity: string): void {
        if (this._hubConnection) {
            this._hubConnection.invoke('LeaveGroup', equity);
        }
    }
    ...
    private init() {
        ...
        this._hubConnection = new signalR.HubConnectionBuilder()
            .withUrl('https://localhost:44324/newsHub')
